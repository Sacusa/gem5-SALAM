...
for app_mix in app_mixes:
    ...
    for policy in policies:
        ...
        for i in range(3):
            exec_time[i] = max(exec_time[i])
        wcet[policy].append(max(exec_time))

    # normalize the values
    norm_value = wcet['LAX'][-1]
    for policy in policies:
        wcet[policy][-1] /= norm_value

# calculate geomean
for policy in policies:
    wcet[policy].append(geo_mean(wcet[policy]))
...
plt.ylabel('WCET (norm. to LAX)', fontsize=30)
...
plt.savefig('../plots/comb_3_repeat/wcet.pdf', bbox_inches='tight')
