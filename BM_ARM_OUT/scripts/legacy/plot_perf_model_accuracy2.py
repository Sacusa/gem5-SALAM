...
for policy in policies:
    ...
    for acc in acc_id_to_name:
        ...
        norm_total = [((load_time[acc][i] + compute_time[acc][i] + \
                store_time[acc][i]) / est_total)
                for i in range(1, len(load_time[acc][1:]))]

        avg_error[policy].append(geo_mean(norm_total))

x = [i for i in range(len(acc_id_to_name))]
x_labels = acc_id_to_name
...
plt.ylabel('Accelerator execution time\n(norm. to perf model)', fontsize=35)
