...
typedef struct {
    ...
    float *o2;
    float *o3;
} micro_set_2_b2_data_t;

// Dummy tasks with output sizes in the name
task_struct_t *micro_set_2_task_36;
task_struct_t *micro_set_2_task_65536;

void add_micro_set_2_b1(task_struct_t **nodes)
{
    micro_set_2_b1_data_t *u1 = (micro_set_2_b1_data_t*)
        get_memory(sizeof(micro_set_2_b1_data_t));
    ...
    for (int i = 0; i < 2; i++) {
        ...
        task[i]->dag_deadline = 925;
    }

    task[0]->status = REQ_STATUS_READY;
    task[0]->num_parents = 1;
    task[0]->producer[0] = micro_set_2_task_65536;
...
void add_micro_set_2_b2(task_struct_t **nodes)
{
    micro_set_2_b2_data_t *u2 = (micro_set_2_b2_data_t*)
        get_memory(sizeof(micro_set_2_b2_data_t));
    ...
    for (int i = 0; i < 3; i++) {
        ...
        task[i]->producer[0] = micro_set_2_task_65536;
        ...
        task[i]->dag_deadline = 151;

        nodes[i] = task[i];
    }

    task[0]->status = REQ_STATUS_READY;
    task[0]->node_deadline = 75;

    task[1]->status = REQ_STATUS_WAITING;
    task[1]->node_deadline = 120;

    task[2]->status = REQ_STATUS_WAITING;
    task[2]->node_deadline = 151;
}

void init_micro_set_2()
{
    micro_set_2_task_36 = (task_struct_t*) get_memory(sizeof(task_struct_t));
    micro_set_2_task_36->output_size = 36;
    micro_set_2_task_36->status = REQ_STATUS_COMPLETED;

    micro_set_2_task_65536 = (task_struct_t*)get_memory(sizeof(task_struct_t));
    micro_set_2_task_65536->output_size = 65536;
    micro_set_2_task_65536->status = REQ_STATUS_COMPLETED;
}

void add_micro_set_2_dags(task_struct_t ***nodes, int *num_nodes)
{
    add_micro_set_2_b1(nodes[0]);
    add_micro_set_2_b2(nodes[1]);
...
int main(int argc, char *argv[])
{
    ...
    init_micro_set_2();
    add_micro_set_2_dags(&nodes[0], &num_nodes[0]);

    runtime(nodes, 2, num_nodes, ELF, MEM_PRED_NO_PRED, true);
    //runtime(nodes, 2, num_nodes, ELF, MEM_PRED_AVERAGE, true);
