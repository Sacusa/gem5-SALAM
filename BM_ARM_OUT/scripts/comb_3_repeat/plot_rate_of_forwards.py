    ...
    for policy in policies:
        ...
        for line in open(dir_name):
            if 'DAG ID' in line:
                ...
                exec_time[dag_id] = int(line.split()[6]) / 1000_000
            elif 'Finished DAG iterations' in line:
                forwards.append(0)
                colocations.append(0)
        ...
        for i, app in enumerate(app_mix):
            fwd_only.append((forwards[i] - colocations[i]) / exec_time[i])
            forwards[i] /= exec_time[i]
            colocations[i] /= exec_time[i]

        num_forwards[policy].append(harmonic_mean(forwards))
        num_colocations[policy].append(harmonic_mean(colocations))
        forwards_only[policy].append(harmonic_mean(fwd_only))

        if policy in ['HetSched', 'ELF']:
            print(app_mix, policy, forwards, num_forwards[policy][-1])

# calculate geomean
for policy in policies:
    avg_percent_colocations = geo_mean([num_colocations[policy][i] / \
...
plt.savefig('../plots/comb_3_repeat/rate_of_forwards.pdf', bbox_inches='tight')
