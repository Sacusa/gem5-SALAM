...
void driver(uint32_t num_elems, uint32_t arg1_addr, uint32_t arg2_addr,
    ...
    // DMA flags
    volatile uint8_t  * DmaFlags   = (uint8_t  *)(DMA_MMR);
    volatile uint64_t * DmaRdAddr  = (uint64_t *)(DMA_MMR + 1);
    volatile uint64_t * DmaWrAddr  = (uint64_t *)(DMA_MMR + 9);
    volatile uint32_t * DmaCopyLen = (uint32_t *)(DMA_MMR + 17);
