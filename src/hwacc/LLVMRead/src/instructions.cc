...
void
FPToUI::compute() {
    // Assuming we are converting from float to unsigned int since it is the
    // more common case and there is no way to tell :/
    float OriginalVal;
    ...
    setResult(&_Result);
    if (_debug) DPRINTF(LLVMOp, "FPToUI:(%s) %llu from %f\n", _OriginalType,
            _Result, OriginalVal);
}
void
FPToSI::compute() {
    ...
    if (_OriginalType == "i64") {
        ...
        if (_debug) DPRINTF(LLVMOp, "FPToSI:(%s) %lld from %f\n",
        ...
        if (_debug) DPRINTF(LLVMOp, "FPToSI:(%s) %ld from %f\n",
                _OriginalType, ResultVal, OriginalVal);
    }
