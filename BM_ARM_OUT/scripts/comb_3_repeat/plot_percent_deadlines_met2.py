...
for app_mix in app_mixes:
    ...
    for policy in policies:
        ...
        iterations = []
        dag_deadlines_met[policy].append([])
        node_deadlines_met[policy].append([])

        for line in open(dir_name):
            if 'Finished DAG iterations' in line:
                iterations.append(int(line.split()[6]))
            elif 'DAG deadlines met' in line:
                value = min(int(line.split()[8]), iterations[-1])
                dag_deadlines_met[policy][-1].append(value)
                node_deadlines_met[policy][-1].append(0)
            elif 'node deadlines met' in line:
                node_deadlines_met[policy][-1][-1] += int(line.split()[8])

        #for i, app in enumerate(app_mix):
        #    num_nodes = iterations[i] * tot_num_nodes[app]
        #    dag_deadlines_met[policy][-1][i] /= (iterations[i] / 100)
        #    node_deadlines_met[policy][-1][i] /= (num_nodes / 100)

        #dag_deadlines_met[policy][-1] = geo_mean(dag_deadlines_met[policy][-1])
        #node_deadlines_met[policy][-1] = geo_mean(node_deadlines_met[policy][-1])

        num_nodes = 0
        for i, app in enumerate(app_mix):
            num_nodes += iterations[i] * tot_num_nodes[app]
        dag_deadlines_met[policy][-1] = (sum(dag_deadlines_met[policy][-1]) * \
                100) / sum(iterations)
        node_deadlines_met[policy][-1] = (sum(node_deadlines_met[policy][-1])*\
                100) / num_nodes
