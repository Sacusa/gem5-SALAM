...
for policy in policies:
    for app_mix in app_mixes:
        ...
        max_forwards = sum([num_edges[app] for app in app_mix])
        max_deadlines_met = sum([num_nodes[app] for app in app_mix])

        if policy == 'LAST_VAL':
            dir_name = '../../comb_3_opt_flush_opt_fwd/' + app_mix_str + \
                    'ELF/debug-trace.txt'
        else:
        ...
        for line in open(dir_name):
            if 'Predicted memory time / byte' in line:
                ...
                forwards[policy].append(int(line.split()[6]))
                #forwards[policy].append(
                #        (int(line.split()[6]) / max_forwards) * 100)

            elif 'Number of node deadlines' in line:
                deadlines_met[policy].append(int(line.split()[8]))
                #deadlines_met[policy].append(
                #        (int(line.split()[8]) / max_deadlines_met) * 100)

        accuracy[policy].append(((error[0] - error[1]) / error[1]) * 100)

with open('../plots/comb_3/bw_predictor_accuracy_performance.csv', mode='w') \
        as csv_file:
    ...
    for i in range(len(app_mixes)):
        ...
        row = [app_mix_str] + \
                ...
                [str(forwards[p][i]) for p in policies] + \
                [str(deadlines_met[p][i]) for p in policies]
