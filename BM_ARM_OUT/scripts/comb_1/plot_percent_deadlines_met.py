...
def add_plot(offset, stat_value, policy, label):
    plt.bar([i+offset for i in x], stat_value, edgecolor=edgecolors[policy],
            width=width, label=label, fc=colors[policy], hatch=hatch[policy],
            zorder=1)
    plt.bar([i+offset for i in x], stat_value, fc='none', edgecolor='k',
