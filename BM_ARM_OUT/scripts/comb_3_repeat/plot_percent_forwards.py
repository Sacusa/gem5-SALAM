...
for app_mix in app_mixes:
    ...
    for policy in policies:
        ...
        for i, app in enumerate(app_mix):
            norm_value += num_edges[app] * iterations[i]
            
            forwards[i] = sum(forwards[i])
            colocations[i] = sum(colocations[i])

        num_forwards[policy].append((sum(forwards) * 100) / norm_value)
        num_colocations[policy].append((sum(colocations) * 100) / norm_value)
