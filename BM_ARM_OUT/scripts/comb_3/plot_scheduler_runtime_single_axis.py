...
def add_avg_plot(offset, policy, label):
    if policy in hatch:
        ax.bar([i+offset for i in x], avg_latency[policy], edgecolor='k',
...
def add_tail_plot(policy, label):
    ax.plot(x, tail_latency[policy], linestyle[policy], linewidth=2,
...
ax.set_xlabel('Application mix', size=35)
ax.set_xticks(x)
ax.set_xticklabels(x_labels, size=35, rotation='vertical')
ax.set_ylabel('Latency (us)', size=35)
ax.set_ylim([0, 2])
ax.yaxis.set_major_locator(plt.MultipleLocator(0.25))
ax.yaxis.set_tick_params(labelsize=35)

ax.grid(zorder=0, color='silver', linestyle='-', linewidth=1)
h, l = ax.get_legend_handles_labels()
ax.legend(h, l, loc="upper left", ncol=4, fontsize=35)
